...
%Note: this function is meant to be used on the output of splitAssays.m
%This will take in a Mx1 cell where each row represents data from a
%different assay from a SINGLE zebrafish. This function will then take the data,
...
%     legend('orig','resample5','resample25'); title(['Assay ' num2str(i)]);
    avg = mean(temp); variance = var(temp);
    AUC5 = trapz(temp);
    feat_orig(i,:) = [avg variance AUC5];
    assay = num2str(i);
    names_orig(i,:) = {['Avg_' assay],['Var_' assay],['AUC5_' assay]};
end

%First pass combinations (within feature combinations)
feat_expand = feat_orig; names_expand = names_orig;
combos_first = combntns(1:3,2);
